<template>
    <div class="containere">
      <h2>Heures de vol et nombre de vols PNT par secteur</h2>
      ...
      <table v-if="pntStats.length" class="result-table">
        ...
        <tbody>
          <tr v-for="stat in pntStats" :key="stat.matricule_pn">
            <td>{{ stat.nom }}</td>
            <td>{{ stat.prenom }}</td>
    ...
    methods: {
      async fetchPNTStats() {
        try {
          ...
          const response = await axios.get("http://localhost:5000/api/vols/stats/pnt", {
            params: {
